...
#         out += f"{atom.SetAtomicNum(atom_types[i])} {coords[i, 0]:.3f} {coords[i, 1]:.3f} {coords[i, 2]:.3f}\n"
#     print(out)
#     exit()
...
# def write_xyz_file(coords, atom_types, filename):
#     out = f"{len(coords)}\n\n"  # 文件头，包含原子数量
#     assert len(coords) == len(atom_types)  # 确保坐标和原子类型数量一致
    
#     mol = ob.OBMol()  # 创建新的分子对象
#     mol.BeginModify()  # 开始修改分子
#     # t = atom_types[i]
#     for i in range(len(coords)):
#         t = atom_types[i]
#         atom = mol.NewAtom()  # 每次循环创建新的原子对象
        
#         atom.SetVector(coords[i][0], coords[i][1], coords[i][2])  # 设置原子的坐标
#         # 输出信息，注意这里需要获取原子符号
#         out += f"{'atom.SetAtomicNum(t)'} {coords[i][0]:.3f} {coords[i][1]:.3f} {coords[i][2]:.3f}\n"
...
def write_xyz_file(coords, atom_types, filename):
    ...
    # 元素周期表映射
    atomic_symbols = {1: 'H', 6: 'C', 7: 'N', 8: 'O', 15: 'P', 16: 'S', 17: 'Cl', 9: 'F', 34: 'Se'}  # 根据需要添加更多元素
    
    for i in range(len(coords)):
        t = atom_types[i]
        symbol = atomic_symbols.get(t, 'X')  # 如果没有找到对应元素，使用 'X'
        out += f"{symbol} {coords[i][0]:.3f} {coords[i][1]:.3f} {coords[i][2]:.3f}\n"
...
def remove_isolated_atoms(mol):
    ...
    for atom in mol.GetAtoms():
        if atom.GetDegree() == 0:  # 没有连接的原子
            removable_atoms.append(atom.GetIdx())
    
    # 创建新分子并添加非孤立原子
    if removable_atoms:
        new_mol = Chem.RWMol(mol)
        for atom_idx in sorted(removable_atoms, reverse=True):
...
#     """
#     Build an RDKit molecule using openbabel for creating bonds and then optimizing the geometry.
#     Args:
#         positions: N x 3
#         atom_types: N
#         atom_decoder: maps indices to atom types
...
#         ff.Setup(ob_mol)
#         ff.ConjugateGradients(500)  # 执行500步几何优化
#         ff.GetCoordinates(ob_mol)

#         # 输出优化后的 sdf 文件
#         obConversion.WriteFile(ob_mol, tmp_file)

#         # 使用 RDKit 读取优化后的 sdf 文件
#         mol = Chem.SDMolSupplier(tmp_file, sanitize=False)[0]

#         if mol is None:
#             raise ValueError("Failed to generate molecule from positions and atom types.")
